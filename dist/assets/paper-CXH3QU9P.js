import{a as e,i as t,n,o as r,r as i,s as a,t as o}from"./legacy-runtime-D6k03fMo.js";var s=`<main id="app-content" class="app-content is-hidden">
        <button id="back-to-library-button" class="button" type="button">
          ←返回文章和讨论列表
        </button>
        <section id="library-view" class="content-grid">
          
          <div class="library-panel-header">

            <div id="library-panel-tabs" class="library-panel-tabs" role="tablist" aria-label="阅读区切换">
              <button
                id="library-panel-reader-button"
                class="library-panel-tab active"
                type="button"
                role="tab"
                aria-selected="true"
              >
                阅读与批注
              </button>
              <button
                id="library-panel-discussion-button"
                class="library-panel-tab"
                type="button"
                role="tab"
                aria-selected="false"
              >
                讨论板
                <span id="discussion-count" class="annotation-count">（0）</span>
              </button>
            </div>
          </div>

          <section class="paper-column">
            <div id="annotation-root" class="paper-card">
              <div class="paper-meta">
                <div class="paper-meta-header">
                  <div>
                    <p id="paper-journal" class="paper-journal">请选择一篇文献</p>
                    <h2 id="paper-title" data-annotation-scope="title">
                      返回列表选择文献后开始阅读
                    </h2>
                  </div>
                  <div class="paper-meta-actions">
                    <a
                      id="source-link"
                      class="source-link is-disabled"
                      href="#"
                      target="_blank"
                      rel="noreferrer"
                      aria-disabled="true"
                    >
                      原文网址
                    </a>
                    <button id="delete-paper-button" class="ghost-button danger-button" type="button" disabled>
                      删除文献
                    </button>
                  </div>
                </div>
                <p id="paper-authors" class="paper-authors" data-annotation-scope="authors"></p>
                <p id="paper-published" class="paper-published"></p>
                <p id="paper-owner" class="paper-owner"></p>
                <p id="paper-abstract" class="paper-abstract" data-annotation-scope="abstract"></p>
                <div id="paper-keywords" class="keyword-list"></div>
              </div>

              <article id="article-root" class="article-root" data-annotation-scope="body">
                <div class="empty-state">
                  当前还没有选中文献。抓取成功后，文献详情和网页快照会从运行时存储目录加载。
                </div>
              </article>
            </div>
          </section>

          <button
            class="pane-resizer"
            type="button"
            data-resizer="right"
            aria-label="调整右侧栏宽度"
            aria-orientation="vertical"
          ></button>

          <aside class="sidebar">
            <section class="panel">
              <div class="panel-header">
                <h3>新建批注</h3>
                <span id="selection-status" class="status-pill">未选择文本</span>
              </div>
              <p class="panel-tip">
                可在标题、作者、摘要或正文中选中一段文字，再填写批注并保存。批注会按当前文献 ID 关联保存。
              </p>
              <textarea
                id="annotation-input"
                rows="2"
                placeholder="例如：这里的实验设计可以和我们组的样地数据对照。"
              ></textarea>
              <div class="attachment-composer">
                <label class="field">
                  <span>附件（支持图片与表格，可多选）</span>
                  <input
                    id="annotation-attachments"
                    type="file"
                    accept="image/*,.csv,.tsv,.xls,.xlsx,.ods"
                    multiple
                  />
                </label>
                <div class="attachment-composer-actions">
                  <p class="panel-tip attachment-tip">
                    支持 PNG、JPG、GIF、WEBP、BMP、CSV、TSV、XLS、XLSX、ODS。
                  </p>
                  <button
                    id="clear-annotation-attachments-button"
                    class="ghost-button"
                    type="button"
                  >
                    清空附件
                  </button>
                </div>
                <div id="annotation-attachments-preview" class="composer-attachment-preview empty-state">
                  还没有选择附件。
                </div>
              </div>
              <div class="composer-actions">
                <button id="add-annotation-button" class="primary-button" disabled>
                  添加批注
                </button>
                <button id="cancel-annotation-button" class="ghost-button" type="button" disabled>
                  取消批注
                </button>
              </div>
            </section>

            <section class="panel">
              <div class="panel-header">
                <h3>当前文献批注</h3>
                <span id="annotation-count" class="annotation-count">0 条</span>
                <button id="clear-storage-button" class="ghost-button" type="button">
                  清空我在当前文献的批注
                </button>
              </div>
              <div id="annotation-list" class="annotation-list empty-state">
                还没有批注。
              </div>
            </section>

            <section class="panel">
              <div class="panel-header">
                <h3>批注详情</h3>
                <div class="panel-actions">
                  <button id="edit-annotation-button" class="ghost-button" type="button" disabled>
                    编辑
                  </button>
                  <button id="delete-annotation-button" class="ghost-button danger-button" type="button" disabled>
                    删除
                  </button>
                </div>
              </div>
              <div id="annotation-detail" class="annotation-detail empty-state">
                点击高亮或右侧列表项后，这里会显示批注内容与讨论线程。
              </div>
              <div id="reply-panel" class="reply-panel">
                <p id="reply-context" class="panel-tip">选择一条批注后可在这里继续讨论。</p>
                <textarea
                  id="reply-input"
                  rows="2"
                  placeholder="例如：我同意这条判断，补充一个方法上的解释。"
                  disabled
                ></textarea>
                <div class="attachment-composer">
                  <label class="field">
                    <span>附件（支持图片与表格，可多选）</span>
                    <input
                      id="reply-attachments"
                      type="file"
                      accept="image/*,.csv,.tsv,.xls,.xlsx,.ods"
                      multiple
                      disabled
                    />
                  </label>
                  <div class="attachment-composer-actions">
                    <p class="panel-tip attachment-tip">可补充图片证据、表格数据或截图。</p>
                    <button
                      id="clear-reply-attachments-button"
                      class="ghost-button"
                      type="button"
                    >
                      清空附件
                    </button>
                  </div>
                  <div id="reply-attachments-preview" class="composer-attachment-preview empty-state">
                    还没有选择附件。
                  </div>
                </div>
                <button id="add-reply-button" class="primary-button" type="button" disabled>
                  回复当前批注
                </button>
              </div>
            </section>
          </aside>

          <section id="discussion-board" class="discussion-board is-hidden">
            <section class="panel">
              <div class="panel-header">
                <h3>发起讨论</h3>
                <span id="discussion-status" class="status-pill">请选择文献</span>
              </div>
              <p class="panel-tip">讨论针对整篇文章，不需要选中文本。</p>
              <textarea
                id="discussion-input"
                rows="2"
                placeholder="例如：这篇文章的实验设计还有哪些可优化的地方？"
              ></textarea>
              <div class="attachment-composer">
                <label class="field">
                  <span>附件（支持图片与表格，可多选）</span>
                  <input
                    id="discussion-attachments"
                    type="file"
                    accept="image/*,.csv,.tsv,.xls,.xlsx,.ods"
                    multiple
                  />
                </label>
                <div class="attachment-composer-actions">
                  <p class="panel-tip attachment-tip">适合补充截图、结果图或数据表。</p>
                  <button
                    id="clear-discussion-attachments-button"
                    class="ghost-button"
                    type="button"
                  >
                    清空附件
                  </button>
                </div>
                <div id="discussion-attachments-preview" class="composer-attachment-preview empty-state">
                  还没有选择附件。
                </div>
              </div>
              <div class="composer-actions">
                <button id="add-discussion-button" class="primary-button" disabled>
                  发布讨论
                </button>
                <button id="cancel-discussion-button" class="ghost-button" type="button" disabled>
                  清空输入
                </button>
              </div>
            </section>

            <section class="panel">
              <div class="panel-header">
                <h3>当前文献讨论</h3>
              </div>
              <div id="discussion-list" class="annotation-list empty-state">
                还没有讨论。
              </div>
            </section>

            <section class="panel">
              <div class="panel-header">
                <h3>讨论详情</h3>
                <div class="panel-actions">
                  <button id="edit-discussion-button" class="ghost-button" type="button" disabled>
                    编辑
                  </button>
                  <button id="delete-discussion-button" class="ghost-button danger-button" type="button" disabled>
                    删除
                  </button>
                </div>
              </div>
              <div id="discussion-detail" class="annotation-detail empty-state">
                选择一条讨论后，这里会展示讨论内容与回复。
              </div>
              <div id="discussion-reply-panel" class="reply-panel">
                <p id="discussion-reply-context" class="panel-tip">
                  选择一条讨论后可在这里继续回复。
                </p>
                <textarea
                  id="discussion-reply-input"
                  rows="2"
                  placeholder="例如：我补充一下另一个角度的看法。"
                  disabled
                ></textarea>
                <div class="attachment-composer">
                  <label class="field">
                    <span>附件（支持图片与表格，可多选）</span>
                    <input
                      id="discussion-reply-attachments"
                      type="file"
                      accept="image/*,.csv,.tsv,.xls,.xlsx,.ods"
                      multiple
                      disabled
                    />
                  </label>
                  <div class="attachment-composer-actions">
                    <p class="panel-tip attachment-tip">回复里也可以补充图片或表格附件。</p>
                    <button
                      id="clear-discussion-reply-attachments-button"
                      class="ghost-button"
                      type="button"
                    >
                      清空附件
                    </button>
                  </div>
                  <div
                    id="discussion-reply-attachments-preview"
                    class="composer-attachment-preview empty-state"
                  >
                    还没有选择附件。
                  </div>
                </div>
                <button id="add-discussion-reply-button" class="primary-button" type="button" disabled>
                  回复当前讨论
                </button>
              </div>
            </section>
          </section>
        </section>

        <section id="profile-view" class="profile-grid is-hidden">
          <section class="panel profile-overview">
            <div class="panel-header">
              <h3>个人信息</h3>
              <span id="profile-stats" class="status-pill">加载中...</span>
            </div>
            <div id="profile-summary" class="profile-summary empty-state">
              登录后可查看你的账号信息。
            </div>
            <div class="profile-action-stack">
              <p class="panel-tip">在账号设置里可以修改你的登录密码。</p>
              <button id="account-settings-button" class="primary-button" type="button">
                账号设置
              </button>
            </div>
          </section>

          <section class="panel profile-annotations-panel">
            <div class="panel-header">
              <div id="profile-panel-tabs" class="profile-panel-tabs" role="tablist" aria-label="我的内容分类">
                <button
                  id="profile-panel-papers-button"
                  class="profile-panel-tab active"
                  type="button"
                  role="tab"
                  aria-selected="true"
                >
                  我上传的文章
                  <span id="my-paper-count" class="annotation-count">0 篇</span>
                </button>
                <button
                  id="profile-panel-speeches-button"
                  class="profile-panel-tab"
                  type="button"
                  role="tab"
                  aria-selected="false"
                >
                  我的发言
                  <span id="my-annotation-count" class="annotation-count">0 条</span>
                </button>
                <button
                  id="profile-panel-replies-button"
                  class="profile-panel-tab"
                  type="button"
                  role="tab"
                  aria-selected="false"
                >
                  别人回复我
                  <span id="received-reply-count" class="annotation-count">0 条</span>
                </button>
              </div>
            </div>

            <section
              id="profile-panel-papers"
              class="profile-panel-view"
              role="tabpanel"
              aria-labelledby="profile-panel-papers-button"
            >
              <p class="panel-tip">按文章上传时间排序，点击“详情”可跳转到对应文章及批注。</p>
              <div id="my-paper-list" class="annotation-list empty-state">
                你还没有上传自己的文章。
              </div>
            </section>

            <section
              id="profile-panel-speeches"
              class="profile-panel-view is-hidden"
              role="tabpanel"
              aria-labelledby="profile-panel-speeches-button"
            >
              <p class="panel-tip">按最新发言时间排序，点击“详情”可跳转到对应文章和发言。</p>
              <div id="my-annotation-list" class="annotation-list empty-state">
                你还没有创建自己的发言。
              </div>
            </section>

            <section
              id="profile-panel-replies"
              class="profile-panel-view is-hidden"
              role="tabpanel"
              aria-labelledby="profile-panel-replies-button"
            >
              <p class="panel-tip">按最新回复时间排序，点击“详情”可跳转到对应文章和这条回复。</p>
              <div id="received-reply-list" class="annotation-list empty-state">
                目前还没有人回复你。
              </div>
            </section>
          </section>
        </section>

        <section id="password-view" class="account-view is-hidden">
          <section class="panel account-panel">
            <div class="panel-header panel-header-actions">
              <h3>修改密码</h3>
              <button id="password-back-button" class="ghost-button" type="button">
                返回
              </button>
            </div>
            <span id="password-status" class="status-pill">请输入当前密码和新密码</span>
            <form id="password-form" class="paper-form">
              <label class="field">
                <span>当前密码</span>
                <input
                  id="current-password"
                  name="currentPassword"
                  type="password"
                  autocomplete="current-password"
                  required
                />
              </label>

              <label class="field">
                <span>新密码</span>
                <input
                  id="next-password"
                  name="nextPassword"
                  type="password"
                  autocomplete="new-password"
                  required
                />
              </label>

              <label class="field">
                <span>确认新密码</span>
                <input
                  id="confirm-password"
                  name="confirmPassword"
                  type="password"
                  autocomplete="new-password"
                  required
                />
              </label>

              <button id="change-password-button" class="primary-button" type="submit">
                更新密码
              </button>
            </form>
          </section>
        </section>

        <section id="members-view" class="profile-grid is-hidden">
          <section class="panel">
            <div class="panel-header">
              <h3>课题组成员</h3>
              <span id="member-count" class="status-pill">0 人</span>
            </div>
            <p class="panel-tip">这里展示除你之外的其他成员，点击任意成员即可查看其上传的文章和发言。</p>
            <div id="member-list" class="paper-list empty-state">
              暂无其他成员。
            </div>
          </section>

          <section class="panel profile-annotations-panel">
            <div class="panel-header">
              <div class="profile-panel-tabs" role="tablist" aria-label="成员内容分类">
                <button
                  id="member-profile-papers-button"
                  class="profile-panel-tab active"
                  type="button"
                  role="tab"
                  aria-selected="true"
                >
                  TA 上传的文章
                  <span id="member-profile-paper-count" class="annotation-count">0 篇</span>
                </button>
                <button
                  id="member-profile-speeches-button"
                  class="profile-panel-tab"
                  type="button"
                  role="tab"
                  aria-selected="false"
                >
                  TA 的发言
                  <span id="member-profile-annotation-count" class="annotation-count">0 条</span>
                </button>
              </div>
            </div>

            <section
              id="member-profile-papers"
              class="profile-panel-view"
              role="tabpanel"
              aria-labelledby="member-profile-papers-button"
            >
              <p class="panel-tip">按文章上传时间排序，点击“详情”可跳转到对应文章。</p>
              <div id="member-profile-paper-list" class="annotation-list empty-state">
                请选择一位成员。
              </div>
            </section>

            <section
              id="member-profile-speeches"
              class="profile-panel-view is-hidden"
              role="tabpanel"
              aria-labelledby="member-profile-speeches-button"
            >
              <p class="panel-tip">按最新发言时间排序，点击“详情”可跳转到对应文章和发言。</p>
              <div id="member-profile-annotation-list" class="annotation-list empty-state">
                请选择一位成员。
              </div>
            </section>
          </section>
        </section>
      </main>
`,c=`/home/dictation/papershare/src/client/detail/DetailPage.jsx`;function l(){return e(a,{children:[e(t,{},void 0,!1,{fileName:c,lineNumber:9,columnNumber:7},this),e(`div`,{className:`page-shell`,children:[e(i,{},void 0,!1,{fileName:c,lineNumber:11,columnNumber:9},this),e(n,{html:s},void 0,!1,{fileName:c,lineNumber:12,columnNumber:9},this)]},void 0,!0,{fileName:c,lineNumber:10,columnNumber:7},this)]},void 0,!0)}var u=`/home/dictation/papershare/src/client/detail/main.jsx`,d=document.getElementById(`app`);if(!d)throw Error(`Detail root container was not found.`);r(e(l,{},void 0,!1,{fileName:u,lineNumber:11,columnNumber:8},void 0),d),o();