...
let clientState: ClientState = createInitialClientState();
...
export function updateClientState(partial: Partial<ClientState> | ((state: ClientState) => Partial<ClientState>)): ClientState {
...
export function createInitialCatalogState(currentUser: User | null = null): CatalogState {
  return {
...
export function createInitialMembersState(): MembersState {
  return {
    allUsers: [],
    groupMembers: [],
    selectedMemberId: "",
...
export function mergeClientState(baseState: ClientState, partialState: Partial<ClientState> = {}): ClientState {
  return {
    ...baseState,
    ...partialState,
...
export function applyAuthenticatedState(user: User, loginStatus: string) {
...
export function applyLoggedOutState(loginStatus: string) {
...
export function resolveDetailFocusState(annotations: Annotation[], discussions: Discussion[], options: { focusAnnotationId?: string, focusReplyId?: string, focusDiscussionId?: string, focusDiscussionReplyId?: string } = {}) {
  ...
  return {
    selectedAnnotationId: options.focusAnnotationId && topLevelAnnotations.has(options.focusAnnotationId)
      ? options.focusAnnotationId
      : null,
    selectedReplyId: options.focusReplyId && annotationIds.has(options.focusReplyId) ? options.focusReplyId : null,
    annotationNavigationTargetId: options.focusAnnotationId && topLevelAnnotations.has(options.focusAnnotationId)
      ? options.focusAnnotationId
      : null,
    selectedDiscussionId: options.focusDiscussionId && topLevelDiscussions.has(options.focusDiscussionId)
      ? options.focusDiscussionId
      : null,
    selectedDiscussionReplyId: options.focusDiscussionReplyId && discussionIds.has(options.focusDiscussionReplyId)
      ? options.focusDiscussionReplyId
      : null,
    discussionNavigationTargetId: options.focusDiscussionId && topLevelDiscussions.has(options.focusDiscussionId)
      ? options.focusDiscussionId
      : null,
  };
}

export function requiresPasswordChange(user: User | null = clientState.auth.currentUser): boolean {
  return Boolean(user?.mustChangePassword);
}

export function normalizeCatalogView(
  viewName: string,
  user: User | null = clientState.auth.currentUser
): CatalogView {
  if (!user) {
...
export function isCurrentUserAdmin(user: User | null = clientState.auth.currentUser): boolean {
  return isAdminUser(user);
}

export function enterPasswordChangeRequiredState(user?: User) {
  const nextUser = user
    ? {
        ...user,
...
export function navigateToClientUrl(url: string) {
  navigateToUrl(url);
}
...
export function resetClientStoreForTests(overrides: Partial<ClientState> = {}) {
  clearPersistedSession();
...
export function setClientStateForTests(partial: Partial<ClientState>) {
  updateClientState(partial);
}

export function setNavigationHandlerForTests(handler: (url: string) => void) {
  navigateToUrl = typeof handler === "function" ? handler : defaultNavigateToUrl;
}

function createInitialClientState(): ClientState {
...
function mergeDetailState(baseDetailState: DetailState, partialDetailState: Partial<DetailState> = {}): DetailState {
  return {
    ...baseDetailState,
    ...partialDetailState,
...
function readApiBaseUrlFromStorage(): string {
  try {
    if (typeof window === "undefined") {
      return "";
    }
...
function defaultNavigateToUrl(url: string) {
  if (typeof window === "undefined") {
    return;
  }
